...
@section('title', 'Leave-Credit-Card-Form')
...
<div id="section">
  ...
  <h2>LEAVE CREDIT CARD FORM</h2>
  </div>
  <div class="container" id="container">
    <form action="{{ route('Admin-Leave-Credit-Card-Generate-page') }}" method="POST">
      @csrf
      ...
      <label for="employee">Select Employee</label>
        <select id="employee" name="employee" required>
            ...
            <option value="{{$employee->email}}">{{ ucfirst(strtolower($employee->fname)) }} {{ $employee->mname ? ucfirst(strtolower($employee->mname)) . '.' : '' }} {{ ucfirst(strtolower($employee->lname)) }}</option>
             @endforeach
        </select>
        </div>
        <div class="form-group">
